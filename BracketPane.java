...
public class BracketPane extends GridPane {
    ...
    public void setUpGrids(){
        leftTopBracket.setPrefSize(400,250);
        leftBottomBracket.setPrefSize(400, 250);
        winnersBracket.setPrefSize(400, 100);
        rightTopBracket.setPrefSize(400, 250);
        rightBottomBracket.setPrefSize(400,250);
        loosersBracket.setPrefSize(400, 100);
        title.setPrefSize(200, 100);
        title.add(new Label("Fifa"), 0, 0, 1, 1);
        loosersBracket.add(this.emptySpace(30, 30), 0, 0, 3, 3);
        winnersBracket.add(this.emptySpace(30,30), 0, 0,3,3);
        for(int i=0;i<3;i++){
            for(int j=0;j<3;j++){
                if(i==1 && j==0){
                    this.add(title, i, j);
                    ...
                    loosersBracket.add(l, i, j);
                    winnersBracket.add(w,i,j);
                ...
                if(!(i==2 && j==2)){
                    loosersBracket.add(this.emptySpace(30, 30), i, j);
                    winnersBracket.add(this.emptySpace(30,30),i,j);
                }
            }
        }
        
        l.setBorder(new Border(new BorderStroke(Color.BLACK, BorderStrokeStyle.DOTTED, CornerRadii.EMPTY, new BorderWidths(1))));
        w.setBorder(new Border(new BorderStroke(Color.BLACK, BorderStrokeStyle.DOTTED, CornerRadii.EMPTY, new BorderWidths(1))));
        ...
        l.add(this.emptySpace(60, 30), 0, 0, 3, 5);
        w.add(this.emptySpace(30, 30),0 ,0,3,5 );
        for(x=0;x<5;x++){
            for(y=1;y<9;y++){
                switch (x) {
                    case 0:
                        ...
                        if(y<4){
                            w.add(new BracketNode("      ",60,30),x,y);
                            l.add(new BracketNode("     ",60,30),x,y);
                        }
                    ...
                    case 1:
                        ...
                        switch (y) {
                            case 2:
                                w.add(teams.get(28), x, y);
                                l.add(teams.get(31), x, y);
                                break;
                            case 4:
                                w.add(teams.get(29), x, y);
                                l.add(teams.get(32), x, y);
                                break;
                            default:
                                if(y<4){
                                    w.add(new BracketNode("      ",60,30),x,y);
                    ...
                    case 2:
                        switch (y) {
                            case 3:
                                ...
                                w.add(teams.get(30), x, y);
                                l.add(teams.get(33), x, y);
